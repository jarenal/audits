    ...
    var formAgents = new Vue({
        ...
        methods: {
            ...
            save: function(event) {
                ...
                if (data.id) {
                    var parameters = { agent: data };

                    $.ajax({
                        url: Routing.generate('api_agents_put', {slug: data.id}),
                        method: 'PUT',
                        data: parameters,
                        cache: false,
                        ...
                        success: function (response, textStatus, jqXHR) {
                            ...
                            window.location.href = Routing.generate('agents_list');
                        },
                        beforeSend: function () {
                    ...
                    $.ajax({
                        url: Routing.generate('api_agents_post'),
                        method: 'POST',
                        data: parameters,
                        cache: false,
                        error: function (jqXHR, textStatus, errorThrown) {
                            // TODO Handle exception
                            console.error('Error on request to api_agents_post', errorThrown)
                        },
                        success: function (response, textStatus, jqXHR) {
                            console.log('success!', response);
                            window.location.href = Routing.generate('agents_list');
